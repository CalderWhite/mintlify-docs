...
The orderbook is reconstructed from multiple redundant websockets to ensure zero data
...
If you plan on downloading high volumes of orderbook data, please reach out! We
...
def download_orderbook_data(market_slug, outcome, date_str, api_key):
...
if __name__ == "__main__":
    ...
    download_orderbook_data(market_slug, outcome, date_str, api_key)
```
...
async fn download_orderbook_data(
    market_slug: &str,
    outcome: &str,
    date_str: &str,
    api_key: &str,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    download_orderbook_data(market_slug, outcome, date_str, api_key).await?;
    Ok(())
}
```
...
```bash cURL
curl -G "http://datasets.predictiondata.dev/polymarket/books/ramp-ipo-in-2025/YES/2025-01-15.csv.gz" \
  --data-urlencode "slug=true" \
  --data-urlencode "apikey=YOUR_API_KEY" \
  --output ramp-ipo-in-2025_YES_2025-01-15.csv.gz
```
...
def download_orderbook_data_by_token(token_id, date_str, api_key):
...
if __name__ == "__main__":
    ...
    download_orderbook_data_by_token(token_id, date_str, api_key)
```
...
async fn download_orderbook_data_by_token(
    token_id: &str,
    date_str: &str,
    api_key: &str,
) -> Result<(), Box<dyn std::error::Error>> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    download_orderbook_data_by_token(token_id, date_str, api_key).await?;
    Ok(())
}
```
...
```bash cURL
curl -G "http://datasets.predictiondata.dev/polymarket/books/21742633143463906290569050155826241533067272736897614950488156847949938836455/2025-01-15.csv.gz" \
  --data-urlencode "apikey=YOUR_API_KEY" \
  --output 21742633143463906290569050155826241533067272736897614950488156847949938836455_2025-01-15.csv.gz
```
